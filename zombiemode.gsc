...
chalk_one_up()
{
	// level.chalk_hud2 = create_chalk_hud( 64 );
	huds = [];
	//huds[1] = create_chalk_hud( 64 );

	chalkSet = level.round_number / 5 //full sets of chalk
	
	// for each whole set of chalk, we'll add the hud for it
	for ( i=0 ; i < chalkSet ; i++ ) 
	{
		huds[i] = create_chalk_hud( 64 * i );
		huds[i] SetShader( "hud_chalk_5", 64, 64 );
	}

	//after calculating how many complete sets of chalk there are, we still have to get the remaining tally
	currentChalk = level.round_number % 5
	if ( !(currentChalk == 0) )
	{
		huds[chalkSet] = create_chalk_hud( 64 * chalkSet );
		huds[chalkSet] SetShader( "hud_chalk_" + ( currentChalk ), 64, 64 );
	}
